...
def lancaPresenca(colunaMatriculas):
    ...
    with open("chamadas/naoLancadas.txt", 'r') as file:
        for line in file:
            ...
            dia = linhaAtual[1]
            codigo = linhaAtual[0]
            with open("chamadas/presencas lancadas/" +
                      dia + ".txt", 'a') as file2:
...
def lancaCadastro(colunaMatriculas, colunaCodigos):
    a = 0  # célula da planilha

    numLinha = 1
    with open("cadastrar/cadastro.txt", 'r') as file,\
            open("chamadas/naoLancadas.txt", 'a') as file2:
...
operacao = raw_input("Insira a operacao a ser efetuada "
                     "(lancarPresenca = 1, lancarCadastro = 2):\n")
if operacao == "1":
